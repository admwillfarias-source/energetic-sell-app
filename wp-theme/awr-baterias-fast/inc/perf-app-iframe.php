...
function awrf_app_iframe_shortcode( $atts ) {
    ...
    $resize_js = '';
    if ( ! $resize_emitted ) {
        $resize_js = "<script>(function(){window.addEventListener('message',function(e){var d=e.data;if(!d||typeof d!=='object'||d.type!=='awr:height')return;var f=document.querySelector('iframe[data-awr-app=\"1\"][src*=\"'+(d.origin||'')+'\"]')||document.querySelector('iframe[data-awr-app=\"1\"]');if(f&&typeof d.height==='number'&&d.height>200){f.style.height=Math.ceil(d.height)+'px';}});})();</script>";
        $resize_emitted = true;
    }
